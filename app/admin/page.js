              ...
              <div key={listing.id} className="rounded-lg border border-[#2A2C3D] bg-[#14151F] p-3">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <p className="text-sm truncate">{listing.products?.name}</p>
                    <p className="text-xs text-[#8A8C9C]">{listing.stores?.name}</p>
                  </div>
                ...
                <form action={updateListingAction} className="flex items-center gap-2">
                  <input type="hidden" name="listing_id" value={listing.id} />
                  <input
                    name="price" type="number" step="0.01" defaultValue={listing.current_price}
                    className="w-24 px-2 py-1.5 rounded-lg bg-[#1E2030] border border-[#2A2C3D] text-sm placeholder-[#5C5E70] focus:outline-none focus:border-[#E8A33D]"
                  />
